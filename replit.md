...
- **WhatsApp Broadcast Module**: Allows sending auction listings to WhatsApp community groups with QR code authentication, group management, and dispatch history. Supports community announcement groups. **Multi-property queue + scheduling**: users can build a queue of multiple properties (via the embedded listing widget or by ID), edit each item's message and optional datetime, then either fire all immediately or schedule for future execution. Backend endpoints `POST /api/whatsapp/disparar-multi` and `POST/GET/DELETE /api/whatsapp/agendamentos` use snake_case `grupo_ids` at the request top level (shared across all items in the request). Background worker (`server/whatsapp-scheduler.ts`) ticks every 60s, processes due agendamentos sequentially, re-reads each item before claiming "executando" (so cancellations win the race), and requeues items as "pendente" if WhatsApp disconnects mid-execution. Requires the Directus collection `whatsapp_agendamentos` (fields: leilao_id, leilao_nome, grupo_ids JSON, mensagem, scheduled_at, status enum, sent_count, failed_count, executed_at, error_message; plus the standard auto-managed fields `date_created` and `date_updated`). Endpoints surface a friendly hint asking to create the collection if missing.
